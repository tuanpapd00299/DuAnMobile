...
public class Mausac_adapter extends BaseAdapter {
	...
	public Mausac_adapter(Context mContext, List<ItemsList> listDanhSach) {
		this.mContext = mContext;
		mInflater = LayoutInflater.from(this.mContext);
		list = listDanhSach;
	}

	public Mausac_adapter() {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		holder.Play.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Uri audio = Uri.parse(""+db.getVoice(10).get(position));
				try {
					playSound(audio);
				} catch (IOException e) {
